...
def download_one_image(bucket, split, image_id, download_folder):
  """Download a single image; be resilient to 404/NotFound errors.

  Returns:
    True if the image was downloaded successfully.
    False if the image was missing (404/NotFound) and was skipped.

  Any other client error is raised to fail fast.
  """
  try:
    ...
    code = error.get('Code', '')
    # Be robust to common not-found codes; just warn and continue.
    if code in ('404', 'NoSuchKey', 'NotFound'):
      print(
          f'WARNING: missing image `{split}/{image_id}` '
          f'(skipping; error code={code})',
          file=sys.stderr)
      return False
    # For other errors, fail loudly as before.
    raise
...
def download_all_images(args):
  ...
  with futures.ThreadPoolExecutor(
    ...
    for future in futures.as_completed(all_futures):
      # Propagate non-404 errors; ignore missing images.
      future.result()
